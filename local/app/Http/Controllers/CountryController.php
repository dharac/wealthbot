...
class CountryController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		// {
		// 	$sql .= 'UPDATE deposit SET approvd_dt='.$value->created_at.' WHERE depositid='.$value->depositid.';<br>';
		// }
		// echo $sql;
		// exit;
		...
		{
			$q = $request->q;
			$countries = Country::orwhere('counm', 'like','%'.$q.'%')
			->orwhere('cou_prefix', 'like','%'.$q.'%')
			->orwhere('cou_code', 'like','%'.$q.'%')
			->paginate($perPage);
		}
		return view('admin.pages.country', compact('countries','q'));
	}
