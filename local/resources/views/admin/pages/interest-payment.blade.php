...
@if($dateRange != "")
	<div class="padding"><div class="col-md-12">{{ $page_name }} show between <b>{{ $dateRange }}</b> &nbsp;&nbsp;<a href="{{ url('admin/interest-payment') }}" class="text-danger" title="Clear Filter"><i class="fa fa-times text-danger"></i></a> </div></div>
@endif
<div class="padding">
	<div class="box">
		<div class="row p-a">
			<div class="col-md-7 text-left row">
				<div class="box-header box-header-data" data-stdt="{{ $stdt }}" data-endt="{{ $endt }}">
					<h2>{{ ucwords($page_name) }} ( <b>Total $ {{ number_format($totalAmt,2) }}</b> )</h2>
				</div>
			</div>
			<div class="col-md-5 text-right">
			{!! Form::open(array('url' => 'admin/interest-payment','method' => 'get')) !!}
			...
			<table class="table table-striped b-t">
				...
				<tbody>
					...
					<tr class="{{ $primary_table_row.$primary_column }}" title="{{ dispayTimeStamp($singleRecord->created_at)->toDayDateTimeString() }}">
						<th scope="row">{{ $a }}</th>
						<td><a class="text-info" href="{{ URL('admin/user/view/'.$singleRecord->id.' ') }}">{{ ucfirst($singleRecord->first_name).' '.ucfirst($singleRecord->last_name) }} | {{ $singleRecord->username }}</a><br>
						<span class="text-primary">{{ $singleRecord->plan_name }}</span>
						</td>
						<td nowrap="nowrap">
						...
						<td>$ {{ number_format($singleRecord->pro_amount,2) }}</td>
					</tr>
					<?php $a++; ?>
					@endforeach
					@else
