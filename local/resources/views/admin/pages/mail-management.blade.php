...
$page_name2 = 'Mail Management';
$allRecord = $mailManagements;
$signature_record = $mail_signatures;
$primary_key = 'mailid';
                ...
                <tbody>
                    ...
                    @if(count($signature_record) > 0)
                        @foreach($signature_record as $singleRecord)
                            <tr title="mail_signature_{{ $singleRecord->$primary_key }}">
                            <th scope="row">{{ $index }}</th>
                            <td><a href="{{ URL('admin/'.$page_name.'/edit/'.$singleRecord->$primary_key.' ') }}" data-placement="left" title="Edit Record">{{ str_limit(strip_tags($singleRecord->body),50) }}<a></td>
                            <td title="{{ $singleRecord->updated_at->diffForHumans() }}" nowrap="nowrap">{{ $singleRecord->updated_at->toDayDateTimeString() }}</td>
                        <td nowrap="nowrap">
                        <a href="{{ URL('admin/'.$page_name.'/edit/'.$singleRecord->mailid.' ') }}" data-placement="left" title="Edit Record"><i class="material-icons">&#xE254;</i></a>
                        </td>
                            </tr>
                        <?php $index++; ?>
                        @endforeach
