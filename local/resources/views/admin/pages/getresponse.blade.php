			...
			<table class="table table-striped b-t">
				...
				<tbody>
					...
					<tr class="{{ $primary_table_row.$primary_column }}" title="{{ $singleRecord->created_at->toDayDateTimeString() }}">
						<th scope="row">{{ $a }}</th>
						<td><a href="{{ URL('admin/'.$page_name.'/edit/'.$primary_column.' ') }}" data-placement="left" title="Edit Record">{{ $singleRecord->username }}</a></td>
						<td>{{ $singleRecord->campaignId }}</td>
						<td>{{ $singleRecord->fromFieldId }}</td>
						<td>{{ $singleRecord->getresponse_api_key }}</td>
						<td nowrap="nowrap">
						...
						<td nowrap="nowrap" title="{{ $singleRecord->updated_at->diffForHumans() }}">{{ $singleRecord->updated_at->toDayDateTimeString() }}</td>
						<td nowrap="nowrap">
						@if($singleRecord->status == 'active')
